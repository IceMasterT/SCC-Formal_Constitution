...
- `u8`, `u32`, and `u64` are big-endian; `u64` values are bounded to `2^53 - 1` so every supported carrier (including IEEE-754-backed JSON parsers) preserves them exactly, and both checkers reject values above the bound.
