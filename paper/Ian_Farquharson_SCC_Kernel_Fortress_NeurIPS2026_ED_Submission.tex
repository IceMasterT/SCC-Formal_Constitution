...
The artifact is fully reproducible from the public repository at \url{https://github.com/IceMasterT/SCC-Formal_Constitution}.
The release gate is:
\begin{lstlisting}
python tools/validate_vectors.py
python tools/forbidden_imports.py
...
bash scripts/run_formal_gates.sh
\end{lstlisting}
The repository includes committed binary canonical vectors (\texttt{.bin}), a Docker image, and a CI workflow that executes the same commands on a Rust- and Node-equipped runner.
All code is released under Apache~2.0; paper and fixture corpus under CC~BY~4.0; canonical documents under CC~BY-SA~4.0.
\end{document}
