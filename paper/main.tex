...
\usepackage{hyperref}
\hypersetup{hidelinks,pdfauthor={Anonymous Authors},pdftitle={SCC Kernel Fortress: Executable Evaluation Gates for Runtime Compliance in Agentic Systems}}
\usepackage{url}
\usepackage{booktabs}
\usepackage{array}
\usepackage{tabularx}
...
\begin{abstract}
Stateful agentic systems operate through tool-use loops, memory updates, external calls, and audit traces, yet existing mechanisms do not reject invalid runtime transitions before state acceptance. We present \KF, an executable evaluation framework that implements a deterministic pre-acceptance checker over a previous state, candidate next state, canonical event, audit event, proof-obligation bundle, and execution descriptor. A transition is accepted only when its evidence satisfies the implemented \SCC\ obligations: canonical encoding agreement, audit-chain consistency, protected-coordinate isolation, governance-state validity, risk-to-halt enforcement, halt absorption, lineage continuation, and normalized \Exact/\Stutter/\SafeRefined\ step classification. The release includes a Rust source-of-truth checker, TypeScript reference checker, golden vectors with binary canonical bytes, a 14-case negative corpus with named first-failure gates, property-test scaffolding, fuzz targets, a Lean/Coq formal bridge for the implemented v1 subset, federation evidence fixtures, numerical-stability bounds, a trusted computing base ledger, and CI release gates. \KF\ does not claim general AI safety, full \SCC\ theorem mechanization, or compiler, operating-system, or cryptographic correctness. As an Evaluations \& Datasets artifact, it contributes a reusable protocol and fixture corpus for testing transition-legality claims in stateful agentic runtimes. \KF\ is the executable boundary artifact; the Full \SCC\ Canon is the formal authority it points back to.
\end{abstract}
...
\begin{lstlisting}[caption={Rust source-of-truth checker public signature.}]
pub fn accepts(
...
\end{align}
The v1 safety preorder is defined coordinatewise:
\begin{equation}
  s' \preceq_{safe} t \;\iff\;
  \mathrm{coord}(s') = \mathrm{coord}(t)
  \;\wedge\; \textstyle\sum_i r_i(s') \leq \textstyle\sum_i r_i(t)
  \;\wedge\; \mathrm{mode}(s') \geq \mathrm{mode}(t),
\end{equation}
where $\mathrm{coord}$ denotes the structural coordinates (compute root, memory root, vector clock, governance weights, protected root), $r_i$ are risk-vector components, and $\mathrm{mode}$ is failure-mode severity (higher is more severe). A bundle whose declared mode disagrees with the classifier is rejected.
...
In \emph{Proceedings of the 28th International Conference on Automated Deduction (CADE-28)}, pages 625--635, 2021.

\bibitem[{The Coq Development Team}(2024)]{coq}
{The Coq Development Team}.
\emph{The {Coq} Proof Assistant, version 8.19}.
Zenodo, 2024. \url{https://doi.org/10.5281/zenodo.10616045}

\bibitem[Anonymous(2026a)]{scc-canon}
Anonymous. \emph{Synthetic Cognitive Coding (SCC): Formal Constitutional-Operational Specification, Final Canon, Scholarly Edition}. Anonymized supplemental manuscript, 2026.

\bibitem[Anonymous(2026b)]{scc-handbook}
Anonymous. \emph{Synthetic Cognitive Coding (SCC): Engineering Handbook, Hardened Builder Edition}. Anonymized supplemental manuscript, 2026.
